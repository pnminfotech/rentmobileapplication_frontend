...
function expenseScopeLabel(item) {
  const type = item.scopeType || (item.propertyType === "room" || item.propertyType === "shop" ? item.propertyType : "hostel");
  const scopeName = item.scopeName || item.buildingName || "";
  if (type === "hostel") return scopeName ? `Hostel: ${scopeName}` : "Hostel expense";
  if (type === "shop") return `${scopeName || "Shop"}${item.roomNo ? ` | Shop ${item.roomNo}` : ""}`;
...
export default function ExpensesScreen() {
  ...
  const visibleItems = useMemo(() => {
    ...
    return rangeItems.filter((item) => {
      ...
      if (category !== "all" && parts[0] !== category) return false;
      if (!term) return true;
      return [...parts, item.roomNo, item.propertyType, item.scopeType, item.scopeName, item.buildingName].some((value) => String(value || "").toLowerCase().includes(term));
    });
  }, [category, rangeItems, search, status]);

  const total = useMemo(() => visibleItems.reduce((sum, item) => sum + Number(item.mainAmount || 0), 0), [visibleItems]);
  const pending = useMemo(() => visibleItems.filter((item) => item.status !== "paid").reduce((sum, item) => sum + Number(item.mainAmount || 0), 0), [visibleItems]);

  function confirmDelete(item) {
    Alert.alert("Delete expense?", `${expenseParts(item)[0] || "This expense"} will be removed.`, [
    ...
    <View style={styles.screen}>
      <View style={styles.content}>
        <View style={styles.header}>
          ...
          <View style={styles.headerText}><Text style={styles.title}>Other expenses</Text><Text style={styles.subtitle}>{monthLabel(selectedMonth)} | {visibleItems.length} entries</Text></View>
        </View>
        ...
        <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.categoryRow}>
            {categories.map((item) => <Pressable key={item} onPress={() => setCategory(item)} style={[styles.categoryButton, category === item && styles.categoryActive]}><Text style={[styles.categoryText, category === item && styles.categoryTextActive]}>{item === "all" ? "All categories" : item}</Text></Pressable>)}
          </ScrollView>
          ...
          <View style={styles.searchBox}>
            ...
            <TextInput value={search} onChangeText={setSearch} placeholder="Search category, notes or unit" style={styles.searchInput} />
          </View>

          {loading ? <View style={styles.loading}><ActivityIndicator size="large" color={colors.primary} /></View> : null}
          {error ? <Text style={styles.error}>{error}</Text> : null}
          {!loading && !error && !visibleItems.length ? <View style={styles.empty}><ReceiptText size={34} color={colors.subtle} /><Text style={styles.emptyTitle}>No expenses</Text><Text style={styles.emptyText}>Change filters or add an expense.</Text></View> : null}

          {visibleItems.map((item) => {
            const parts = expenseParts(item);
            return (
              <View key={item._id} style={styles.row}>
                <View style={styles.rowMain}><Text style={styles.rowTitle}>{parts[0] || "Expense"}</Text><Text style={styles.rowMeta}>{parts.slice(1).join(" | ") || "General"} | {expenseScopeLabel(item)}</Text>{auditMeta(item) ? <Text style={styles.auditMeta}>{auditMeta(item)}</Text> : null}</View>
                <View style={styles.amountBox}><Text style={styles.amount}>{money(item.mainAmount)}</Text><Text style={[styles.status, item.status === "paid" && styles.paid]}>{item.status || "pending"}</Text></View>
                <View style={styles.actionStack}>
                  <Pressable onPress={() => router.push({ pathname: "/system/expense-form", params: { id: item._id } })} style={styles.actionButton}><Pencil size={17} color={colors.primary} /></Pressable>
                  <Pressable onPress={() => router.push({ pathname: "/system/audit-history", params: { entityType: "otherExpense", entityId: item._id, title: parts[0] || "Expense", returnTo: "/system/expenses" } })} style={styles.actionButton}><History size={17} color={colors.primary} /></Pressable>
        ...
        <Pressable onPress={() => router.push("/system/expense-form")} style={styles.fab}><Plus size={24} color={colors.surface} /></Pressable>
      </View>
    </View>
  );
}
...
const styles = StyleSheet.create({
  ...
  categoryButton: { minHeight: 36, paddingHorizontal: 12, alignItems: "center", justifyContent: "center", borderWidth: 1, borderColor: colors.border, borderRadius: 7, backgroundColor: colors.surface },
  categoryActive: { borderColor: colors.border, backgroundColor: colors.primarySoft },
